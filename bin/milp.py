...
def initialize_run_dofp(seg_bitrates, qualities, throughput, buffer, save_buffer_t, seg_length, time_current_segment,
    ...
    # Check variables
    # print("available_bitrates={}, available_qualities={}, quality_array={}, estimated_throughput={:.3f}, current_buffer={:.3f}, save_buffer_threshold={:.3f}, total_remaining_time={:.3f},segment_duration={:d}, remaining_time_current_segment={:.3f}, available_time_next_segment={:.3f}, retrans_strategy={:d}\n".format(available_bitrates, available_qualities, quality_array, estimated_throughput, current_buffer, save_buffer_threshold, total_remaining_time, segment_duration, remaining_time_current_segment, available_time_next_segment, retrans_strategy))
    # RUN DOFP #
    return run_dofp()
    ...
    if quality_array[-1] != 0:
        ...
        if len(gap_index_array_sorted) == 1:  # no gap in the buffer
            ...
            for q_n in available_qualities[1:]:  # r_n is the quality for the next segment
                ...
                # print("*************************************************\n==> in quality {}, t_download {}".format(q_n, t_download))
                    ...
                    for gap_id in range(1, len(gap_index_array_sorted)):
                        ...
                        for seg_id in reversed(gap_index_array_sorted[gap_id]):
                            ...
                            for retrans_quality_id in range(quality_array[seg_id] + 1,
                                ...
                                # print("\t\t==> retrans_quality_id {}. bitrate {}".format(retrans_quality_id, available_bitrates[retrans_quality_id]))
                                ...
                                if t_download[
                                    -1] > available_time or total_download_time > total_remaining_time:  # can't retransmit this segment
                                    # go to next gap
                                    # print("##################################### On no, next gap ###########")
                                    next_gap_flag = True
                                    break
                                else:
